 ...
 */
public class Day_10 extends Day_00 {
	private final String INPUT_FILE = "files\\day_10_input.txt";
	private final int NUMBER_LIST_LENGTH = 256;

	// Part 2
	private final String SUFFIX = "17, 31, 73, 47, 23";
	...
	@Override
	// Some help from:
	// https://github.com/karthikb351/advent-of-code-2017/blob/master/Day%2010%20-%20Knot%20Hash.ipynb
	public void solvePart1() {
		List<String> lines = getLines(INPUT_FILE);

		// Initialize the list of numbers
		int[] elements = new int[NUMBER_LIST_LENGTH];
		...
		int skipSize = 0;
		String[] lengths = lines.get(0).replaceAll("\t| ", "").split(",");
		for (String strLength : lengths) {
			int length = Integer.parseInt(strLength);
			if (length <= elements.length) {

				int[] subarray = getSubArray(elements, currentPosition, length);
				subarray = reverse(subarray);
				mergeArrays(currentPosition, elements, subarray);
		...
		System.out.println("Part 1 product: " + (elements[0] * elements[1]));
	}
	...
	public void solvePart2() {
		...
		int[] lengths = new int[line.length() + suffixStr.length];
		...
		for (int i = 0; i < suffixStr.length; i++) {
			lengths[i + line.length()] = Integer.parseInt(suffixStr[i]);
		}
		...
		String[] hexadecimalHash = new String[16];
		for (int i = 0; i < hexadecimalHash.length; i++) {
			hexadecimalHash[i] = Integer.toHexString(denseHash[i]);
			
			if(hexadecimalHash[i].length()<2) {
				hexadecimalHash[i]="0"+hexadecimalHash[i];
			}
		}

		// Convert to a Knot hash string
		String knotHash = "";
		for (String value : hexadecimalHash) {
			knotHash += value;
		}

		System.out.println("Part 2 hash: " + knotHash);
	}
